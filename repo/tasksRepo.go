...
func (r *TaskRepository) Create(ctx context.Context, newTask Task) error {

	tx, err := r.db.BeginTx(ctx, nil)
	defer tx.Rollback()

	_, err = tx.ExecContext(ctx, "INSERT INTO tasks (title, done, user_id) VALUES ($1, $2, $3)", newTask.Title, newTask.Done, newTask.UserID)
	if err != nil {
		return err
	}
